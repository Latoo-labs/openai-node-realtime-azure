...
export class OpenAIRealtimeWS extends OpenAIRealtimeEmitter {
  ...
  constructor(
    props: { model: string; transcription?: boolean; options?: WS.ClientOptions | undefined },
    client?: Pick<OpenAI, 'apiKey' | 'baseURL'>,
  ) {
    super();
    client ??= new OpenAI();

    this.url = buildRealtimeURL(client, props.model, props.transcription);
  ...
  static async azure(
    ...
    options: { deploymentName?: string; transcription?: boolean; options?: WS.ClientOptions | undefined } = {},
  ): Promise<OpenAIRealtimeWS> {
    const deploymentName = options.deploymentName ?? client.deploymentName;
    if (!deploymentName) {
      throw new Error('No deployment name provided');
    }
    const transcription = options.transcription ?? false;
    return new OpenAIRealtimeWS(
      { model: deploymentName, transcription, options: { headers: await getAzureHeaders(client) } },
      client,
    );
  }
